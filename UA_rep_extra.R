...
writeLines("______________Full Rmd Code Run___________\n\n")
writeLines(readLines(bargs$rmdfile))
sink(type="message")
sink()
close(con)
if(rmd_check != "Rmd file could not be run"){
  tryCatch(system(sprintf("\"%s\" -O %s -s Letter -L 10 -R 10 -T 15 -B 20 --zoom 1.3 \"%s\" --header-center \" \" \"%s\"", 
